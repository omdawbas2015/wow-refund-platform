...
type VerifyFn = (req: NextRequest) => NextResponse | null;
async function importVerify(): Promise<VerifyFn> {
  const mod = await import('./auth');
  return mod.verifyCronAuth as VerifyFn;
}
...
describe('lib/cron/auth', () => {
  it('returns a 503 when CRON_SECRET is unset (deployment misconfigured)', async () => {
    ...
    const res = verify(makeReq({ 'x-cron-secret': 'whatever' }));
  ...
  it('accepts the secret in the x-cron-secret header', async () => {
    ...
    const verify = await importVerify();
    expect(verify(makeReq({ 'x-cron-secret': 's3cret' }))).toBeNull();
  });
  ...
  it('rejects with 401 when neither header matches', async () => {
    vi.stubEnv('CRON_SECRET', 's3cret');
    const verify = await importVerify();
    const res = verify(makeReq({ 'x-cron-secret': 'wrong' }));
    expect(res).not.toBeNull();
  ...
  it('rejects when the bearer prefix is wrong (e.g. Token / Basic)', async () => {
    vi.stubEnv('CRON_SECRET', 's3cret');
    const verify = await importVerify();
  ...
  it('treats unequal secret lengths as a non-match (no length leak via crash)', async () => {
    ...
    const verify = await importVerify();
    const res = verify(makeReq({ 'x-cron-secret': 'short' }));
    expect(res?.status).toBe(401);
  });
});
